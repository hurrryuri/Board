...
class BoardServiceImplTest {
    ...
    public void pagelistTest2(){
        ...
        //필요한 값은 size없어도 10 page 없어도 1, type, keyword
        //keyword가 없을때, 있을때,
        ...
        log.info("끝페이지 : " + pageResponseDTO.getEnd());
        log.info("시작페이지 : " + pageResponseDTO.getStart());
        log.info("이전여부 : " + pageResponseDTO.isPrev());
        log.info("다음여부 : " + pageResponseDTO.isNext());

        //if 시작페이지가 있다면, 없다면? 다음페이지가 있다면 없다면,
    }
    ...
    public void testest(){
        ...
        long total = boardPage.getTotalElements();   // 총 게시물수
        log.info(totalpages);
        log.info(total);
    ...
    public void test1(){
        ...
        long total =
        boardPage.getTotalElements(); // 게시물총수
        int totalpage =
        ...
        int end = (int)(Math.ceil(page / 10.0)) * 10;   //페이지마지막출력번호
        log.info(" 현재" + page + "페이지 10개사이즈 그래서 마지막 값은 : ? " +end);
        int start = end -9;
        // 만약에 마지막페이지가 32페이지라고 하면
        // 현재 페이지가 31이다 그럼 지금까지나온 end페이지는 40이 될것이다.
        // 그런데 실제로는 32까지밖에 없으니 그 두값을 비교한다. 실제 마지막페이지를 계산한다.
        int last = (int) Math.ceil(total / (double)size) ;
        log.info("실제 마지막 페이지 : " + last + "페이지");
        ...
        log.info("실제값과 비교해서 나온 진짜 마지막페이지는 ? : " + end);
