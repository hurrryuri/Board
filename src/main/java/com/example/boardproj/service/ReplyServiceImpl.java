...
public class ReplyServiceImpl implements ReplyService{
    ...
    public void register(ReplyDTO replyDTO) {
        ...
        //board.setBno(replyDTO.getBno)로 만든 객체는 임의의 객체이므로
        //저장이 되지 않는다.
    ...
    public List<ReplyDTO> list(Long bno) {
        ...
        //List에서 하나 순차적으로 0부터 꺼내서 꺼낸아이를 변수 reply라 칭하고
        //이걸 가지고 replyDTO타입으로 변환한다음 리턴값으로 ReplyDTO 타입으로
        //받는데 받은 ReplyDTO에 setBoardDTO를 하기위해 아까 받은 reply에서
        //reply.getBoard를 받아서 다시 BoardDTO타입으로 변화해서 set 하기 위해서
        //setBoardDTO의 리턴타입을 replyDTO로 줘서 리턴을 다시 ReplyDTO로 만든다.
        //리턴값이 있어야지만 .collect(Collectors.toList());를 사용할수 있어서
        return list;
    }
    ...
    public Page<ReplyDTO> listPage(Long bno, Pageable pageable) {
        ...
        //select * from table (pageable) 파라미터로 넣어주면 페이지 객체로 반환합니다.
        //15페이지라 하면 10개씩이라하면 10페이지부터 10부터~ 20까지 보여줄 예정
        //내부에서 직접 만들고 page와 size를 받았다면
//        Pageable ex = PageRequest.of((int)(bno -1), 10, Sort.by("rno").descending());
    ...
    public ReplyDTO update(ReplyDTO replyDTO) {
        ...
        //업데이트 수행
        // 수정한 뒤에 reply를 replyDTO로 변환해서 변환을 해야하지만..
